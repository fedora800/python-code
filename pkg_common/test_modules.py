...
def test_fn_download_and_sync_db_loop_for_mult_symbols(sa_engine, session):
  ...
  for symbol in lst_symbols:
    ...
    df_prices = m_udb.fn_get_symbol_price_data_stats_from_database(sa_engine, symbol)
    print("df_xx = ", df_prices)
    df_ohlcv_symbol = m_yfn.fn_sync_price_data_in_table_for_symbol("YFINANCE", sa_engine, symbol, df_prices)
    print(df_ohlcv_symbol)

  # this one using 2 wildcards for specific sql query
  # wildcard_value_1 = "%FA%"
  # wildcard_value_2 = "%SB%"
