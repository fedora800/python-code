from datetime import datetime import inspect import re

import pandas as pd import plotly as pl import plotly.graph_objects as gobj

#import sys #from datetime import * #from plotly.offline import plot # INPUT_FILE='C:\\mytmp\\file-downloads\\im_max_count_2020-02-03_fmtd.csv' #INPUT_FILE_IM = 'C:\\mytmp\\file-downloads\\im_max_count.archive_2020-02-25.prod_archive_fmtd.csv' INPUT_FILE_IM = 'C:\\mytmp\\influxdb_stats\\IM\\im_max_count_2020-07-16.prod_archive_fmtd.csv' #INPUT_FILE_STREAMER = 'C:\\mytmp\\file-downloads\\streamer_max_count_2020-03-09.prod_archive_fmtd.csv' INPUT_FILE_STREAMER = 'C:\\mytmp\\influxdb_stats\\Streamer\\streamer_max_count_2020-07-16.prod_archive_fmtd.csv' OUTPUT_DIR = 'C:\\mytmp\\influxdb_stats' TRADING_HOLIDAYS_AMER = ['2020-01-01', '2020-01-20',
...
# print(traces_list.dict)
 ...
 output_file = OUTPUT_DIR + '/' + 'APAC' + '_' + column_param + '.html' print('Plotting chart for APAC into file ' + output_file) 
 fig = dict(data=traces_list_APAC, layout=layout) pl.offline.plot(fig, filename=output_file, auto_open=False) 
 output_file = OUTPUT_DIR + '/' + 'EMEA' + '_' + column_param + '.html' print('Plotting chart for EMEA into file ' + output_file) 
 # fig = dict(data=traces_list, layout=layout) fig = dict(data=traces_list_EMEA, layout=layout) 
 # # pl.offline.plot(fig) # will display in browser 
 # # saves to a file and does not display to browser pl.offline.plot(fig, filename=output_file, auto_open=False) 
 # # todo - need to look at write_html # https://plotly.github.io/plotly.py-docs/generated/plotly.io.write_html.html 
